...
该特性不稳定，会导致一些网页访问有时出现异常（不是速度慢，应该是因为 目前的tls过滤方式有点问题, 对close_alert等情况没处理好，以及分包没有隔离准确，导致了tls数据包 的一半被加密，一半被裸奔，导致连接异常。而且使用不同的浏览器，现象也会不同，似乎对safari支持好一些， chrome就差很多）；
