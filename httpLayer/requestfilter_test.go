...
func TestGetPath(t *testing.T) {
	...
	_, method, p1, falreason := GetRequestMethod_and_PATH_from_Bytes([]byte(str1), false)
	...
	_, _, p2, falreason := GetRequestMethod_and_PATH_from_Bytes([]byte(str2), false)
	if p2 != "/sdfdsffs.html" {
	...
	_, _, p3, falreason := GetRequestMethod_and_PATH_from_Bytes([]byte(str3), false)
	if p3 == "/x" { //尾缀长度不够
	...
	_, _, p4, failreason := GetRequestMethod_and_PATH_from_Bytes([]byte(str4), true)
	if p4 != requestStr {
